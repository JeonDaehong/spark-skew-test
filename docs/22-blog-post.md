...
셔플이 키를 **해시로** 나눈다. 그래서 같은 키는 반드시 같은 파티션으로 간다.
어떤 키에 행이 몰려 있으면 그 파티션 하나가 비대해진다. 주로 이런 데서 나온다.

- **join** — 특정 고객 ID, 특정 상품 코드에 거래가 몰린다
- **group by** — 집계 키 하나에 원본 행이 몰린다
- **NULL** — NULL 은 전부 같은 파티션으로 간다. 뒤에서 따로 다룬다
